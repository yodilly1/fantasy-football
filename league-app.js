(() => {
  ...
  function renderVotes() {
    ...
    root.innerHTML = app.proposals.map((proposal) => {
      ...
      return `<article class="panel proposal-card"><div class="proposal-meta"><span class="status">${escapeHtml(proposal.category)}</span><span class="status ${['open', 'passed'].includes(proposal.status) ? 'success' : ''}">${escapeHtml(proposal.status)}</span><span class="status ${proposal.alert_error ? 'warning' : ''}">${escapeHtml(emailStatus)}</span></div><h3>${escapeHtml(proposal.title)}</h3><p>${escapeHtml(proposal.description || proposal.proposed_value || '')}</p>${proposal.proposed_value ? `<p style="margin-top:8px"><b>${escapeHtml(proposal.current_value || 'Current')}</b> → <b>${escapeHtml(proposal.proposed_value)}</b></p>` : ''}<div class="vote-tally"><div class="vote-count"><span>Yes · ${proposal.required_yes_votes || majority} needed</span><strong>${yes}</strong></div><div class="vote-count"><span>No</span><strong>${no}</strong></div></div>${proposal.status === 'open' ? `<div class="proposal-actions"><button class="button ${mine === 'yes' ? 'primary' : 'secondary'}" data-vote="${proposal.id}" data-choice="yes">Vote yes</button><button class="button ${mine === 'no' ? 'primary' : 'secondary'}" data-vote="${proposal.id}" data-choice="no">Vote no</button></div>` : ''}${proposal.alert_error && proposal.author_manager_id === app.managerId ? `<button class="text-button proposal-retry" data-retry-alert="${proposal.id}">Retry email alert</button>` : ''}</article>`;
    }).join('');
  }

  function proposalModal() {
    openModal({title: 'New league proposal', copy: 'Any manager can add a rule, side bet, or league change. A majority of managers must vote yes, and passed changes are implemented before the September 9 season kickoff.', submitLabel: 'Open voting', body: `<div class="form-grid"><div class="field full"><label>Title</label><input id="proposal-title" placeholder="Add a weekly side bet" required></div><div class="field"><label>Category</label><select id="proposal-category"><option value="finance">Finance</option><option value="keeper">Keeper</option><option value="scoring">Scoring</option><option value="draft">Draft</option><option value="side_betting">Side betting</option><option value="punishment">Punishment</option><option value="other">Other</option></select></div><div class="field"><label>Effective season</label><input id="proposal-season" type="number" value="2026"></div><div class="field"><label>Current value</label><input id="proposal-current" placeholder="No current rule"></div><div class="field"><label>Proposed value</label><input id="proposal-value" placeholder="₪50 weekly side bet"></div><div class="field full"><label>Reason</label><textarea id="proposal-description" placeholder="Explain the rule, side bet, or change and how it should work"></textarea></div></div>`, onSubmit: async (modal) => {
  ...
  function renderHistory() {
    ...
    const seasonsByYear = [...seasons].sort((a, b) => b.year - a.year), selectedYear = Number(app.historyYear) || Number(seasonsByYear[0]?.year), selected = seasons.find((season) => Number(season.year) === selectedYear) || seasonsByYear[0];
    const selector = $('#history-season');
    selector.innerHTML = seasonsByYear.map((season) => `<option value="${season.year}" ${Number(season.year) === Number(selected.year) ? 'selected' : ''}>${season.year}</option>`).join('');
    selector.onchange = () => { app.historyYear = Number(selector.value); renderHistory(); };
    $('#season-history').innerHTML = seasonsByYear.map((season) => { const top = [...(season.teams || [])].sort((a,b) => Number(b.wins || 0) - Number(a.wins || 0) || Number(b.points || 0) - Number(a.points || 0))[0]; return `<div class="data-row"><div><b>${season.year} · ${championFor(season)}</b><small>${season.year === 2026 ? 'Rollover season in setup' : `${top?.team_name || 'Top team'} · ${top?.wins || 0}–${top?.losses || 0}`} · ${season.transactions?.length || 0} transactions</small></div><span class="status ${season.year < 2026 ? 'success' : 'warning'}">${season.year < 2026 ? 'Complete' : 'Setup'}</span></div>`; }).join('');
    if (!selected) { $('#history-explorer').innerHTML = '<div class="empty-state">No Sleeper season selected.</div>'; return; }
    const standings = [...(selected.teams || [])].sort((a, b) => Number(b.wins || 0) - Number(a.wins || 0) || Number(b.points || 0) - Number(a.points || 0));
    const draftRows = [...(selected.draft_picks || [])].sort((a, b) => Number(a.pick_no || 0) - Number(b.pick_no || 0)).slice(0, 24);
    const latestTransactions = [...(selected.transactions || [])].filter((row) => row.status === 'complete').sort((a, b) => Number(b.created || 0) - Number(a.created || 0)).slice(0, 12);
    $('#history-explorer').innerHTML = `<div class="panel-head"><div><span class="kicker">Season explorer</span><h3>${selected.year} league data</h3><p>Read-only Sleeper snapshot: standings, auction picks, transactions, matchups, and playoff data.</p></div><span class="status">${selected.teams?.length || 0} teams</span></div><div class="history-columns"><div><h4>Standings</h4><div class="table-wrap"><table><thead><tr><th>#</th><th>Team</th><th>W–L</th><th>Points</th></tr></thead><tbody>${standings.map((team, index) => `<tr><td>${index + 1}</td><td><b>${escapeHtml(team.team_name)}</b><small>${escapeHtml(team.display_name || '')}</small></td><td class="numeric">${team.wins || 0}–${team.losses || 0}</td><td class="numeric">${Number(team.points || 0).toFixed(1)}</td></tr>`).join('')}</tbody></table></div></div><div><h4>Draft picks</h4><div class="table-wrap"><table><thead><tr><th>Pick</th><th>Player</th><th>Team</th><th>Price</th></tr></thead><tbody>${draftRows.length ? draftRows.map((pick) => `<tr><td>${pick.pick_no || '—'}</td><td><b>${escapeHtml(`${pick.metadata?.first_name || ''} ${pick.metadata?.last_name || ''}`.trim() || pick.player_id)}</b><small>${escapeHtml(pick.metadata?.position || '')}</small></td><td>${escapeHtml(standings.find((team) => String(team.roster_id) === String(pick.roster_id))?.team_name || `Roster ${pick.roster_id}`)}</td><td class="numeric">${pick.metadata?.amount ? `$${pick.metadata.amount}` : '—'}</td></tr>`).join('') : '<tr><td colspan="4">No draft picks recorded.</td></tr>'}</tbody></table></div></div><div><h4>Recent transactions</h4><div class="rows">${latestTransactions.length ? latestTransactions.map((row) => `<div class="data-row"><div><b>${escapeHtml(row.type || 'transaction')}</b><small>${new Date(Number(row.created || 0)).toLocaleDateString()} · ${row.adds ? `${Object.keys(row.adds).length} adds` : ''}${row.drops ? ` · ${Object.keys(row.drops).length} drops` : ''}</small></div><span class="status ${row.type === 'trade' ? 'success' : ''}">${escapeHtml(row.status || '')}</span></div>`).join('') : '<div class="empty-state">No completed transactions.</div>'}</div></div></div>`;
    const lastYear = seasons.find((season) => Number(season.year) === 2025);
    const expensive = [...(lastYear?.draft_picks || [])].filter((pick) => Number(pick.metadata?.amount) > 0).sort((a, b) => Number(b.metadata.amount) - Number(a.metadata.amount)).slice(0, 12);
    const lastYearTeams = lastYear?.teams || [];
    $('#expensive-players').innerHTML = `<div class="panel-head"><div><span class="kicker">2025 auction records</span><h3>Most expensive players</h3><p>The highest winning bids from last year’s auction, in fantasy dollars.</p></div><span class="status">${expensive.length} records</span></div><div class="table-wrap"><table><thead><tr><th>#</th><th>Player</th><th>Position</th><th>Manager / team</th><th>Winning bid</th></tr></thead><tbody>${expensive.length ? expensive.map((pick, index) => { const name = `${pick.metadata?.first_name || ''} ${pick.metadata?.last_name || ''}`.trim() || pick.player_id; const team = lastYearTeams.find((entry) => String(entry.roster_id) === String(pick.roster_id)); return `<tr><td>${index + 1}</td><td><b>${escapeHtml(name)}</b></td><td>${escapeHtml(pick.metadata?.position || '—')}</td><td>${escapeHtml(team?.team_name || `Roster ${pick.roster_id}`)}<small>${escapeHtml(team?.display_name || '')}</small></td><td class="numeric"><b>$${Number(pick.metadata.amount)}</b></td></tr>`; }).join('') : '<tr><td colspan="5">No auction prices recorded for 2025.</td></tr>'}</tbody></table></div>`;
  }
  ...
  function renderHelp() {
    const cards = [
      ...
      ['Draft time', 'Open Draft time and add as many windows as possible across multiple days.', ['Google Calendar reads busy/free time only; an unverified-app warning is expected for this private league tool.', 'Outlook can be imported from an .ics file.', 'Manual entry supports a date range, selected weekdays, timezone, and Open / Possible / Unavailable.']],
      ...
      ['Money and votes', 'Money is tracked in shekels; auction values are separate fantasy dollars.', ['Record NIS payments or the USD equivalent with the agreed exchange rate.', 'Any manager can propose rules, side bets, or other league changes.', 'A majority vote passes a proposal; approved changes are implemented before the September 9 kickoff.']],
