(() => {
  ...
  function renderMoney() {
    ...
    ledger.innerHTML = obligations.map((obligation) => {
      ...
      return `<div class="data-row"><div><b>${escapeHtml(managerName(obligation.manager_id))} · ${escapeHtml(obligation.description)}</b><small>Due ${obligation.due_at ? new Date(obligation.due_at).toLocaleDateString() : 'before draft'}${settlement ? ` · ${settlement.payment_currency} ${settlement.payment_amount} credited as ₪${Number(settlement.credited_nis).toFixed(2)}` : ''}</small></div><div class="row-actions"><span class="money">₪${Number(obligation.amount_nis).toFixed(0)}</span><span class="status ${status === 'confirmed' ? 'success' : status === 'rejected' ? 'danger' : 'warning'}">${status}</span>${app.isCommissioner && status === 'submitted' ? `<button class="button secondary" data-settlement="${settlement.id}" data-status="confirmed">Confirm</button>` : ''}</div></div>`;
    }).join('');
  }
